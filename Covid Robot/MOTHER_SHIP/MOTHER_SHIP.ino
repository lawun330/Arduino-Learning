...
#define LITER_10      2000

//#define LITER_10      400000

//dispensor pins
#define DISPENSOR_L_PIN   46
#define DISPENSOR_R_PIN   47
...
int STATE_L = LOW; //dispensor
int STATE_R = LOW;
unsigned long lastDebounceTime1=0;
unsigned long lastDebounceTime2=0;
unsigned long debounceDelay=50;
int button_L_state; int button_R_state;
int button_L_last=0;
int button_R_last=0;
...
void setup()
{
  ...
  pinMode(DISPENSOR_L_PIN,OUTPUT);
  pinMode(DISPENSOR_R_PIN,OUTPUT);
...
//Dispensor
  digitalWrite(DISPENSOR_L_PIN,STATE_L);
  digitalWrite(DISPENSOR_R_PIN,STATE_R);
...
char keys()
{
  ...
  else if (x_axis==x_normal and y_axis==y_normal and button_joy==0 and button_L==1 and button_R==0){left(); return 's';}
  else if (x_axis==x_normal and y_axis==y_normal and button_joy==0 and button_L==0 and button_R==1){right(); return 's';}
}
//////////////////////
float battery()
{
...
void left()
{
  if (button_L!=button_L_last){lastDebounceTime1=millis();}
  
  if ((millis()-lastDebounceTime1) > debounceDelay)
  {
    if(button_L!=button_L_state)
    {
      button_L_state = button_L;
      if (button_L_state == HIGH){STATE_L = !STATE_L;}
    }
  }
...
void right()
{
  if (button_R!=button_R_last){lastDebounceTime2=millis();}
  
  if ((millis()-lastDebounceTime2) > debounceDelay)
  {
    if(button_R!=button_R_state)
    {
      button_R_state = button_R;
      if (button_R_state == HIGH){STATE_R = !STATE_R;}
    }
  }
