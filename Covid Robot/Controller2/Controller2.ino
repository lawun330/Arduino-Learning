...
int data=1; //data for connection check
...
{     
  ...
  x_val=map(x_axis,0,1023,0,100);
  y_val=map(y_axis,0,1023,0,100);
}
/////////////////////
void buttons()
{
...
void lcd_joystick()
{
      ...
      else if (x_val==x_normal and y_val==y_normal and button_Joy_val==0 and button_L_val==1 and button_R_val==0)
      {
        t--;
        lcd.setCursor(0,0); 
        lcd.print("REFILLING MODE  "); 
        if (t>0)
        {
          lcd.setCursor(0,1); 
          lcd.print("Hold ");
      ...
      else if (x_val==x_normal and y_val==y_normal and button_Joy_val==0 and button_L_val==0 and button_R_val==1){lcd.setCursor(0,0); lcd.print("                "); lcd.setCursor(0,1); lcd.print("                ");}
...
void radioListen()
{
  ...
  if(radio.available()){radio.read(&voltage,sizeof(voltage)); digitalWrite(led,HIGH);}
  volt_percent();
}
///////////////////
void volt_percent()
...
void print_val()
{
  Serial.print(x_val); //x=50,y=49 at normal value
  Serial.print("\t");
  Serial.print(y_val);
  Serial.print("\t");
